import MySQLdb
import datetime
from sql_module.sql_exception import OpenDataBaseException, CategoryExistException, CategoryNotExistException,\
    IncomeCategoriesNotExist, AllCategoriesNotExist, ExpenseCategoriesNotExist, HistoryNotExist
...
class SQL:
    ...
    def open(self):
        try:
            ...
            self._db = MySQLdb.connect(host=self._host, user=self._user, passwd=self._passwd, db=self._database,
                                       charset=self._charset)
            # формируем курсор
            self._cursor = self._db.cursor()
        except Exception:
            raise OpenDataBaseException
    ...
    # пока просто возвращает !!! сумм и коментарии всех операций за период по всем категориям
